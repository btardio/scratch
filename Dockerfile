...
RUN mkdir /usr/src/foundationdb

WORKDIR /usr/src/foundationdb

ARG foundation_clone_url=https://github.com/apple/foundationdb.git
RUN git clone $foundation_clone_url /usr/src/foundationdb

RUN mkdir /usr/src/foundationdb-builddir

WORKDIR /usr/src/foundationdb-builddir

RUN cmake -G Ninja /usr/src/foundationdb
...
RUN mkdir /usr/include/foundationdb
RUN cp /usr/src/foundationdb-builddir/bindings/c/foundationdb/fdb_c_options.g.h /usr/include/foundationdb
RUN cp /usr/src/foundationdb-builddir/bindingtester/tests/c/foundationdb/fdb_c.h /usr/include/foundationdb

# ARG additional_configure_options=-I/usr/src/foundationdb-builddir/fdbserver/bindingtester/tests/c/foundationdb/

RUN find / -name libfdb_c.so
RUN mkdir /usr/lib/foundationdb
RUN cp /usr/src/foundationdb-builddir/lib/libfdb_c.so /usr/lib/foundationdb

# start couchdb-erlfdb

RUN apt-get -y install rebar
RUN apt-get -y install erlang

WORKDIR /usr/src/
RUN git clone https://github.com/apache/couchdb-erlfdb.git
WORKDIR /usr/src/couchdb-erlfdb
RUN LDFLAGS="-L/usr/lib/foundationdb" rebar compile
...
RUN git fetch origin \
    ...
    && LDFLAGS="-L/usr/lib/foundationdb" CFLAGS="-I/usr/src/foundationdb-builddir/fdbserver/bindingtester/tests/c/foundationdb" make release
...
RUN echo admin = somepassword >> /opt/couchdb/etc/local.ini

#ENTRYPOINT ["tini", "--", "/docker-entrypoint.sh"]
#CMD ["/opt/couchdb/bin/couchdb && /bin/sh"]
CMD ["/bin/bash"]
